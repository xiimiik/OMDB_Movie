...
export const CardItem: React.FC<Props> = ({ imgUrl, title, year, imdbID }) => {
  const [select, setSelect] = useState(false);
  const { favoritesIds, setFavoritesIds } = useContext(UserContext);
  const isFavorite = favoritesIds.find((id) => id === imdbID);
  ...
  const handleFavorite = (event: React.MouseEvent) => {
    ...
    if (isFavorite) {
      const filteredFavorites = favoritesIds.filter((id) => id !== imdbID);

      setFavoritesIds(filteredFavorites);
    } else {
      setFavoritesIds([...favoritesIds, imdbID]);
    }
  ...
  return (
    <Card
      ...
      className="bg-light"
    >
      <Link href={`/movies/${imdbID}`} className="d-flex h-100 flex-column">
        <img
          alt="Movie poster"
          src={poster}
      ...
      <button onClick={handleFavorite} type="button" className="btn btn-dark">
        {select ? (
